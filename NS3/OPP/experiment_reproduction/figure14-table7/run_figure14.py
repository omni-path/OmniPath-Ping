#!/usr/bin/env python3
"""Run Figure 14 metrics and Table 7 under sampled-USM rates.

The runner reuses the Figure 10-11 single-packet traffic loads and evaluates:
...
The runner writes one Figure 14(a) CSV for max FCT:

    sample_rate_percent,fattree_k64,leaf_spine_k64,2d_torus_10x10,3d_torus_6x6x6
    0,<max_fct_ms>,...
    ...

It also writes one Figure 14(b) CSV for the maximum per-switch loopback RX probe count:
...
def build_tasks(args, repo_root, work_root):
    traffic_root = repo_root / "experiment_reproduction" / "figure10-11" / "traffic_load"
...
def write_result_files(results_dir, rows):
    ...
    output_file = results_dir / "Figure14(a)_results.csv"
    ...
    output_file = results_dir / "Figure14(b)_results.csv"
    with output_file.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["sample_rate_percent"] + list(TOPOLOGY_ORDER))
        for sample_rate in sorted(sample_rates):
