#!/usr/bin/env python3
"""Run Figure 15: fat-tree k=64 sampled-USM experiments.
...
def build_tasks(args, repo_root, work_root, part):
    ...
    flow_file = (
        repo_root / "experiment_reproduction" / "figure10-11" /
...
def run_all_parts(args, repo_root, figure_dir, sim_dir, work_root):
    ...
    for part in ("a", "b"):
        ...
        output_files[part] = (
            figure_dir / args.results_dir / ("Figure15(%s)_results.csv" % part)
        ).resolve()
...
def parse_args():
    ...
    parser.add_argument("--part", choices=("a", "b", "all"), default="all",
                        help="which Figure 15 part to run; all runs a then b")
    ...
    parser.add_argument("--qlen-trace-from-peak700", action="store_true",
                        help=("use previous Figure15 peak700 node CSVs to choose "
...
def main():
    ...
    if args.qlen_trace_from_peak700:
        if args.qlen_trace_peak_map_a is None:
            args.qlen_trace_peak_map_a = (
                figure_dir / "results" / "Figure15a_peak700_node_qlen.csv")
        if args.qlen_trace_peak_map_b is None:
            args.qlen_trace_peak_map_b = (
                figure_dir / "results" / "Figure15b_peak700_node_qlen.csv")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure15_")
        ...
        if args.part == "all":
            ...
            output_file = (
                figure_dir / (
                    ...
                    args.results_dir / ("Figure15(%s)_results.csv" % args.part)
                )
            ).resolve()
