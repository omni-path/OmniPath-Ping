#!/usr/bin/env python3
"""Run Figure 7(b)/8(b) forwarding-loop sweeps on k=64 fat-tree."""
...
OPP_SAMPLED_SOURCE_PACKET_EXP = 4
PACKET_EXPS = tuple(range(0, 15))
OPP_SAMPLED_RUN_EXPS = tuple(range(0, OPP_SAMPLED_SOURCE_PACKET_EXP + 1))
SPLIT_START_PACKET_EXP = 12
DEFAULT_OUTPUT_FILE = "Figure7(b)_results.csv"
DEFAULT_F1_OUTPUT_FILE = "Figure8(b)_results.csv"
...
SERIES = (
    ...
    },
    {
        ...
        "run_exps": OPP_SAMPLED_RUN_EXPS,
        "copy_from_exp": OPP_SAMPLED_SOURCE_PACKET_EXP,
    },
    {
        "series": "RDPROBE",
        "mode": "RDPROBE",
...
def build_tasks(args, repo_root, figure_dir, work_root):
    ...
    for series_cfg in SERIES:
        for exp in PACKET_EXPS:
            ...
            copy_from_exp = None
            if exp not in set(series_cfg["run_exps"]):
                copy_from_exp = series_cfg.get("copy_from_exp")
...
def main():
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=None,
                        help="result CSV; default is results/Figure7(b)_results.csv")
    parser.add_argument("--f1-output-file", type=pathlib.Path, default=None,
                        help="F1 CSV; default is results/Figure8(b)_results.csv")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure7b_loop_")
