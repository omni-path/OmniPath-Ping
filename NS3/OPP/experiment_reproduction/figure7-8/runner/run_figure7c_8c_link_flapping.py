#!/usr/bin/env python3
"""Run Figure 7(c)/8(c) link-flapping sweeps on k=64 fat-tree."""
...
PACKET_EXPS = tuple(range(0, 21))
OPP_RUN_PACKET_EXPS = tuple(range(0, 6))
OPP_COPY_SOURCE_PACKET_EXP = 5
SPLIT_START_PACKET_EXP = 12
DEFAULT_OUTPUT_FILE = "Figure7(c)_results.csv"
DEFAULT_F1_OUTPUT_FILE = "Figure8(c)_results.csv"
...
def build_tasks(args, repo_root, figure_dir, work_root):
    ...
    for series_cfg in SERIES:
        for exp in PACKET_EXPS:
            ...
            copy_from_exp = None
            if exp not in series_cfg["run_exps"]:
                if series_cfg["mode"] == "OPP":
                    copy_from_exp = OPP_COPY_SOURCE_PACKET_EXP
                else:
                    raise ValueError("non-OPP series cannot copy packet exponent %d" % exp)
...
def main():
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=None,
                        help="result CSV; default is results/Figure7(c)_results.csv")
    parser.add_argument("--f1-output-file", type=pathlib.Path, default=None,
                        help="F1 CSV; default is results/Figure8(c)_results.csv")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure7c_link_flapping_")
