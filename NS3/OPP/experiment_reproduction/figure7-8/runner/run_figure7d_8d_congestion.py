#!/usr/bin/env python3
"""Run Figure 7(d)/8(d) congestion sweeps on k=64 fat-tree."""
...
PACKET_EXPS = tuple(range(0, 13))
SPLIT_START_PACKET_EXP = 12
DEFAULT_OUTPUT_FILE = "Figure7(d)_results.csv"
DEFAULT_F1_OUTPUT_FILE = "Figure8(d)_results.csv"
...
def main():
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=None,
                        help="result CSV; default is results/Figure7(d)_results.csv")
    parser.add_argument("--f1-output-file", type=pathlib.Path, default=None,
                        help="F1 CSV; default is results/Figure8(d)_results.csv")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure7d_congestion_")
