#!/usr/bin/env python3
"""Run Figure 6 2D-torus coverage sweeps.

The script evaluates RPINGMESH, RDPROBE, and OPP on 2D torus topologies using
the pre-generated `traffic_load/2d_torus/` all-to-all flow files. It runs all required ns-3
...
DEFAULT_OUTPUT_FILE = "Figure6(c)_results.csv"
...
def build_tasks(args, repo_root, figure6_dir, work_root):
    ...
    for k in args.k:
        topology = "%dx%d" % (k, k)
        topology_file = repo_root / "experiment_reproduction" / "topologies" / "2d_torus" / (
            "torus_%s_topology.txt" % topology)
        if not topology_file.exists():
            raise FileNotFoundError("missing topology file: %s" % topology_file)

        flow_dir = figure6_dir / "traffic_load" / "2d_torus" / topology
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run 2D-torus average-coverage sweeps for Figure 6.")
    parser.add_argument("--k", type=int, nargs="+", default=[8, 10],
                        help="2D torus dimensions to run; k means k x k")
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=None,
                        help="coverage-matrix CSV result file; default is results/Figure6(c)_results.csv")
    ...
    parser.add_argument("--opp-m1", type=int, default=1,
                        help="unused for 2D torus; kept for config compatibility")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure6c_")
