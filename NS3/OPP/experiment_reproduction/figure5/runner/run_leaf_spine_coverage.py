#!/usr/bin/env python3
"""Run Figure 5 leaf-spine coverage sweeps.

The script evaluates RPINGMESH, RDPROBE, and OPP on leaf-spine topologies using
the pre-generated `traffic_load/leaf_spine/` first-ToR flow files. It runs all required ns-3
...
DEFAULT_OUTPUT_FILE = "Figure5(b)_results.csv"
...
def build_tasks(args, repo_root, figure5_dir, work_root):
    ...
    for k in args.k:
        topology = "k%d" % k
        topology_file = repo_root / "experiment_reproduction" / "topologies" / "leaf_spine" / (
            "leaf_spine_k%d_topology.txt" % k)
        if not topology_file.exists():
            raise FileNotFoundError("missing topology file: %s" % topology_file)

        flow_dir = figure5_dir / "traffic_load" / "leaf_spine" / topology
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run leaf-spine average-coverage sweeps for Figure 5.")
    parser.add_argument("--k", type=int, nargs="+", default=[32, 64],
                        help="leaf-spine k values to run")
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=None,
                        help="coverage-matrix CSV result file; default is results/Figure5(b)_results.csv")
    ...
    parser.add_argument("--opp-m", type=int, default=1,
                        help="unused for leaf-spine; kept for config compatibility")
    ...
    parser.add_argument("--opp-m1", type=int, default=1,
                        help="leaf-spine OPP intra-ToR tokens per source ToR")
    parser.add_argument("--opp-m2", type=int, default=8,
                        help="leaf-spine OPP inter-ToR tokens per source/destination ToR")
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure5b_")
