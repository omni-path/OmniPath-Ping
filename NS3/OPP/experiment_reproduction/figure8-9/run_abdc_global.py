#!/usr/bin/env python3
"""Run Figure 8/9 subfigures a, b, d, c with one global concurrency limit.
...
GROUP_SPECS = (
    ("a", "run_figure8a_9a_acl_drop.py"),
    ("b", "run_figure8b_9b_loop.py"),
    ("d", "run_figure8d_9d_congestion.py"),
    ("c", "run_figure8c_9c_link_flapping.py"),
)
...
def build_groups(args, repo_root, figure_dir, work_root):
    ...
    for label, script_name in GROUP_SPECS:
        if label not in args.groups:
            continue
        module = load_module("figure8_9_%s" % label, runner_dir / script_name)
...
def run_global(args):
    ...
    if args.work_root is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure8_9_abdc_")
