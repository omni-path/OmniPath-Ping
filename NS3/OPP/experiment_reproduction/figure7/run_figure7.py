#!/usr/bin/env python3
"""Unified runner for Figure 7 workloads."""
...
def build_tasks(repo_root: pathlib.Path, figure7_dir: pathlib.Path, work_root: pathlib.Path, args):
    topo_mode_map = {
        "fattree": "FAT_TREE",
    ...
    for topology_type, topo_key, mode_exps in TOPOLOGY_CONFIGS:
        flow_dir = figure7_dir / "traffic_load" / topology_type / topo_key
...
def main():
    parser = argparse.ArgumentParser(description="Run Figure 7 full-load experiments.")
    ...
    parser.add_argument("--output-file", type=pathlib.Path, default=pathlib.Path("results/Figure7_results.csv"),
    ...
    figure7_dir = pathlib.Path(__file__).resolve().parent
    output_file = (figure7_dir / args.output_file).resolve()

    if args.work_dir is None:
        work_dir = tempfile.TemporaryDirectory(prefix="figure7_")
    ...
    try:
        tasks = build_tasks(repo_root, figure7_dir, work_root.resolve(), args)
