#!/usr/bin/env python3
"""Generate Figure 11(a): fat-tree OPP cache-entry bound vs multiplier."""
...
DEFAULT_OUTPUT_FILE = "Figure11(a)_results.csv"
...
def entry_count(k, multiplier, base_m1, base_m2, base_m3):
    m1 = base_m1 * multiplier
    m2 = base_m2 * multiplier
    m3 = base_m3 * multiplier
    return m1 + m2 * (k // 2) + m3 * k * k
...
def write_matrix(output_file, args):
    ...
    with output_file.open("w", newline="") as f:
        ...
        for k in args.k:
            writer.writerow(
                ["k%d" % k] +
                [entry_count(k, x, args.base_m1, args.base_m2, args.base_m3)
...
def parse_args():
    ...
    parser.add_argument("--base-m1", type=int, default=10,
                        help="base OPP_FATTREE_M1, matching Figure 10(a)")
    parser.add_argument("--base-m2", type=int, default=10,
                        help="base OPP_FATTREE_M2, matching Figure 10(a)")
    parser.add_argument("--base-m3", type=int, default=1,
                        help="base OPP_FATTREE_M3, matching Figure 10(a)")
...
def main():
    args = parse_args()
    if any(k <= 0 or k % 2 != 0 for k in args.k):
        raise SystemExit("--k values must be positive even integers")
