#!/usr/bin/env python3
"""Run Figure 10(c): 2D torus OPP sampled-USM completion time vs m.

The output CSV is a compact matrix:

    series,1,2,...,8
    4x4,<max_fct_ms for m=1>,...
    ...
"""
...
DEFAULT_OUTPUT_FILE = "Figure10(c)_results.csv"
...
def build_tasks(args, repo_root, figure_dir, work_root):
    ...
    for k in args.k:
        topo_key = "%dx%d" % (k, k)
        topology_file = (
            repo_root / "experiment_reproduction" / "topologies" / "2d_torus" /
            ("torus_%s_topology.txt" % topo_key)
        )
        flow_file = figure_dir / "traffic_load" / "2d_torus" / topo_key / "flow_2e0.txt"
...
def parse_args():
    ...
    parser.add_argument("--k", type=int, nargs="+", default=list(DEFAULT_K),
                        help="2D torus dimensions; k means k x k")
...
def main():
    ...
    if args.work_dir is None:
        temp_work_dir = tempfile.TemporaryDirectory(prefix="figure10c_")
