...
static Time
GetRpingmeshQpCompleteDelay(uint32_t ccMode)
{
	return ccMode == RDMA_CC_MODE_RPINGMESH ? MilliSeconds(100) : MilliSeconds(1);
}

static uint32_t
GetRpingmeshPayloadSize(uint32_t headerSize)
{
	return RPINGMESH_PACKET_SIZE > headerSize ? RPINGMESH_PACKET_SIZE - headerSize : 0;
}

static uint32_t
GetOppPayloadSize()
{
	return GetRpingmeshPayloadSize(CustomHeader::GetStaticWholeHeaderSize() + OppProbeHeader::GetStaticSize());
}

static bool
ExtractOppProbeHeader(Ptr<Packet> p, OppProbeHeader &opph)
{
	if (p->GetSize() < CustomHeader::GetStaticWholeHeaderSize() + OppProbeHeader::GetStaticSize())
		return false;
...
static bool
ExtractOppAckHeader(Ptr<Packet> p, OppAckHeader &opph)
{
	uint32_t headerSize = PppHeader::GetStaticSize() + 20 + qbbHeader::GetBaseSize() +
		IntHeader::GetStaticSize() + OppAckHeader::GetStaticSize();
	if (p->GetSize() < headerSize)
		return false;
	Ptr<Packet> copy = p->Copy();
	PppHeader ppp;
	Ipv4Header ip;
...
RdmaHw::RdmaHw(){
	m_oppServerId = 0;
	m_rpingmeshTracerouteOnMissingAck = false;
	m_rpingmeshTracerouteOnLoopFault = false;
	m_rpingmeshTracerouteOnCongestionFault = false;
	m_rpingmeshTracerouteTtl = RPINGMESH_TRACEROUTE_TTL;
}
...
void RdmaHw::SetOppTokenManager(uint32_t probeInstanceId, Ptr<OppTokenManager> manager, uint32_t serverId){
	m_oppTokenManagers[probeInstanceId] = manager;
	m_oppServerId = serverId;
	if (manager != NULL)
		manager->RegisterServer(serverId);
}

void RdmaHw::SetRpingmeshProbeSentCallback(RpingmeshProbeSentCallback cb){
	m_rpingmeshProbeSentCallback = cb;
}

void RdmaHw::SetRpingmeshProbeTtlCallback(RpingmeshProbeTtlCallback cb){
	m_rpingmeshProbeTtlCallback = cb;
}

void RdmaHw::SetRpingmeshAckReceivedCallback(RpingmeshAckReceivedCallback cb){
	m_rpingmeshAckReceivedCallback = cb;
}

void RdmaHw::SetRpingmeshTracerouteOnMissingAck(bool enable){
	m_rpingmeshTracerouteOnMissingAck = enable;
}

void RdmaHw::SetRpingmeshTracerouteOnLoopFault(bool enable){
	m_rpingmeshTracerouteOnLoopFault = enable;
}

void RdmaHw::SetRpingmeshTracerouteOnCongestionFault(bool enable){
	m_rpingmeshTracerouteOnCongestionFault = enable;
}

void RdmaHw::SetRpingmeshTracerouteTtl(uint8_t ttl){
	m_rpingmeshTracerouteTtl = ttl;
}

void RdmaHw::SetOppAckReceivedCallback(OppAckReceivedCallback cb){
	m_oppAckReceivedCallback = cb;
}
...
void RdmaHw::Setup(QpCompleteCallback cb){
	for (uint32_t i = 0; i < m_nic.size(); i++){
		...
		dev->m_rdmaEQ->m_rdmaCanSendQp = MakeCallback(&RdmaHw::CanSendQp, this);
	}
...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp){
	...
	if (it != m_rtTable.end() && it->second.size() > 0)
		return PickRoute(it->second, qp->GetHash());
	if (m_defaultRt.size() > 0)
		return PickRoute(m_defaultRt, qp->GetHash());
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, uint32_t oppRemainingHopCount, uint32_t oppTokenSourceTorId, uint32_t oppTokenDstPodId, uint32_t oppProbeInstanceId, Callback<void> notifyAppFinish){
	// create qp
	NS_ASSERT_MSG(oppRemainingHopCount <= 0xff, "OPP remaining hop count exceeds 8 bits");
	NS_ASSERT_MSG(oppTokenSourceTorId <= 0xffff, "OPP token source ToR ID exceeds 16 bits");
	NS_ASSERT_MSG(oppTokenDstPodId <= 0xff, "OPP token destination PoD ID exceeds 8 bits");
	Ptr<RdmaQueuePair> qp = CreateObject<RdmaQueuePair>(pg, sip, dip, sport, dport);
	qp->m_isRpingmesh = IsRpingmeshLikeCcMode(m_cc_mode);
	qp->m_oppRemainingHopCount = (uint8_t)oppRemainingHopCount;
	qp->m_oppTokenSourceTorId = oppTokenSourceTorId;
	qp->m_oppTokenDstPodId = oppTokenDstPodId;
	qp->m_oppProbeInstanceId = oppProbeInstanceId;
	qp->SetSize(size);
	qp->SetWin(win);
	...
	if (m_cc_mode == RDMA_CC_MODE_OPP){
		Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
		NS_ASSERT_MSG(manager != NULL, "OPP token manager is not configured for this host/instance");
		manager->RegisterQueuePair(
			qp,
			m_oppServerId,
			MakeCallback(&RdmaHw::WakeupOppTokenSender, this),
			MakeCallback(&RdmaHw::OnOppTokenReleased, this));
	}
	...
	if (qp->m_isRpingmesh && qp->m_size == 0 && !qp->m_rpingmeshCompleteScheduled){
		qp->m_rpingmeshCompleteScheduled = true;
		Simulator::Schedule(GetRpingmeshQpCompleteDelay(m_cc_mode), &RdmaHw::QpComplete, this, qp);
	}
}
...
void RdmaHw::DeleteQueuePair(Ptr<RdmaQueuePair> qp){
	if (m_cc_mode == RDMA_CC_MODE_OPP){
		Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
		if (manager != NULL)
			manager->UnregisterQueuePair(qp);
	}
...
uint32_t RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q){
	...
	if (it != m_rtTable.end() && it->second.size() > 0)
		return PickRoute(it->second, q->GetHash());
	if (m_defaultRt.size() > 0)
		return PickRoute(m_defaultRt, q->GetHash());
	NS_ASSERT_MSG(false, "We assume at least one NIC is alive");
	return 0;
}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	if (IsRpingmeshLikeCcMode(m_cc_mode)){
		Ptr<RdmaRxQueuePair> rxQp = GetRxQp(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport, ch.udp.pg, true);
		rxQp->m_ecn_source.total++;

		bool isOpp = m_cc_mode == RDMA_CC_MODE_OPP;
		bool rpingmeshFault = false;
		OppAckHeader oppAck;
		if (isOpp){
			OppProbeHeader oppProbe;
			NS_ASSERT_MSG(ExtractOppProbeHeader(p, oppProbe), "OPP data packet does not carry an OPP probe header");
			oppAck.SetTokenIdA(oppProbe.GetTokenIdP());
			oppAck.SetTokenSeqNoA(oppProbe.GetTokenSeqNoP());
			oppAck.SetEqualCostPathCountA(1);
			oppAck.SetMaximumQDelayA(oppProbe.GetPreviousHopQDelayP());
			oppAck.SetCongestedLinkA(oppProbe.GetPreviousHopLinkP());
			oppAck.SetFaultyLinkA(0);
			oppAck.SetIsLoopA(0);
		}else if (m_cc_mode == RDMA_CC_MODE_RPINGMESH && !m_rpingmeshProbeTtlCallback.IsNull()){
			Ptr<Packet> ttlProbe = p->Copy();
			PppHeader ppp;
			Ipv4Header ip;
			ttlProbe->RemoveHeader(ppp);
			ttlProbe->RemoveHeader(ip);
			rpingmeshFault = m_rpingmeshProbeTtlCallback(ch, ip.GetTtl());
		}
		...
		if (rpingmeshFault)
			seqh.SetRpingmeshFault();

		uint32_t headerSize = PppHeader::GetStaticSize() + 20 + seqh.GetSerializedSize();
		if (isOpp)
			headerSize += OppAckHeader::GetStaticSize();
		Ptr<Packet> newp = Create<Packet>(GetRpingmeshPayloadSize(headerSize));
		if (isOpp)
			newp->AddHeader(oppAck);
		newp->AddHeader(seqh);
		...
		NS_ASSERT_MSG(newp->GetSize() == RPINGMESH_PACKET_SIZE,
			"RPINGMESH/RDPROBE/OPP ACK packet size must stay 100 bytes");
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	bool rpingmeshTracerouteAck = ((ch.ack.flags >> qbbHeader::FLAG_RPINGMESH_TRACEROUTE) & 1) != 0;
	uint32_t rpingmeshTracerouteSwitchId = rpingmeshTracerouteAck ? seq : 0;
	int i;
	Ptr<RdmaQueuePair> qp = GetQp(ch.sip, port, qIndex);
	if (qp == NULL){
		if (m_cc_mode == RDMA_CC_MODE_RPINGMESH){
			if (!m_rpingmeshAckReceivedCallback.IsNull())
				m_rpingmeshAckReceivedCallback(ch.dip, ch.sip, ch.ack.dport, ch.ack.sport,
					qIndex, seq, Simulator::Now().GetTimeStep(), ch.ack.flags,
					rpingmeshTracerouteSwitchId);
			return 0;
		}
		if (IsRpingmeshLikeCcMode(m_cc_mode))
			return 0;
	...
	if (m_cc_mode == RDMA_CC_MODE_OPP){
		uint32_t tokenId = 0;
		uint16_t tokenSeqNo = 0;
		OppAckHeader oppAck;
		NS_ASSERT_MSG(ExtractOppAckHeader(p, oppAck), "OPP ACK packet does not carry an OPP ACK header");
		tokenId = oppAck.GetTokenIdA();
		tokenSeqNo = oppAck.GetTokenSeqNoA();
		if (std::getenv("OPP_ACK_PATH_LOG") != NULL){
			uint32_t dstNode = (qp->dip.Get() >> 8) & 0xffff;
			std::cout << "OPP_ACK_PATH_COUNT src_node " << m_node->GetId()
				<< " dst_node " << dstNode
				<< " sport " << qp->sport
				<< " dport " << qp->dport
				<< " token " << tokenId
				<< " token_seq " << tokenSeqNo
				<< " equal_cost_path_count " << oppAck.GetEqualCostPathCountA()
				<< " faulty_link " << oppAck.GetFaultyLinkA()
				<< " congested_link " << oppAck.GetCongestedLinkA()
				<< " max_qdelay " << oppAck.GetMaximumQDelayA()
				<< std::endl;
		}
		if (!m_oppAckReceivedCallback.IsNull())
			m_oppAckReceivedCallback(qp->sip.Get(), qp->dip.Get(), qp->sport, qp->dport,
				qIndex, oppAck.GetMaximumQDelayA(), oppAck.GetFaultyLinkA(), oppAck.GetIsLoopA());
		Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
		if (manager != NULL){
			bool oldObserved = qp->m_oppProbeLastAckObserved;
			uint64_t oldTimeNs = qp->m_oppProbeLastAckTimeNs;
			qp->m_oppProbeLastAckObserved = true;
			qp->m_oppProbeLastAckTimeNs = Simulator::Now().GetTimeStep();
			if (!manager->OnOppAck(qp, tokenId, tokenSeqNo)){
				qp->m_oppProbeLastAckObserved = oldObserved;
				qp->m_oppProbeLastAckTimeNs = oldTimeNs;
			}
		}
		return 0;
	}
	if (m_cc_mode == RDMA_CC_MODE_RPINGMESH){
		if (rpingmeshTracerouteAck)
			qp->m_rpingmeshTracerouteAckCount++;
		else{
			qp->m_rpingmeshAckCount++;
			if (((ch.ack.flags >> qbbHeader::FLAG_RPINGMESH_FAULT) & 1) != 0)
				qp->m_rpingmeshLoopFaultSeen = true;
			if (m_rpingmeshTracerouteOnCongestionFault){
				std::map<uint32_t, uint64_t>::iterator sendIt =
					qp->m_rpingmeshProbeSendTimeNsBySeq.find(seq);
				if (sendIt != qp->m_rpingmeshProbeSendTimeNsBySeq.end()){
					uint64_t nowNs = Simulator::Now().GetTimeStep();
					uint64_t delayNs = nowNs > sendIt->second ? nowNs - sendIt->second : 0;
					if (delayNs > qp->m_rpingmeshMaxProbeAckDelayNs)
						qp->m_rpingmeshMaxProbeAckDelayNs = delayNs;
					if (delayNs > 300000)
						qp->m_rpingmeshCongestionFaultSeen = true;
					qp->m_rpingmeshProbeSendTimeNsBySeq.erase(sendIt);
				}
			}
		}
		if (rpingmeshTracerouteAck){
			uint64_t nowNs = Simulator::Now().GetTimeStep();
			uint64_t delayNs = nowNs > qp->m_rpingmeshTracerouteStartNs ?
				nowNs - qp->m_rpingmeshTracerouteStartNs : 0;
			qp->m_rpingmeshTracerouteAckCountByCore[rpingmeshTracerouteSwitchId]++;
			uint64_t &maxDelayNs = qp->m_rpingmeshTracerouteMaxDelayByCore[rpingmeshTracerouteSwitchId];
			if (delayNs > maxDelayNs)
				maxDelayNs = delayNs;
		}
		if (!m_rpingmeshAckReceivedCallback.IsNull())
			m_rpingmeshAckReceivedCallback(qp->sip.Get(), qp->dip.Get(), qp->sport, qp->dport,
				qIndex, seq, Simulator::Now().GetTimeStep(), ch.ack.flags,
				rpingmeshTracerouteSwitchId);
		return 0;
	}
...
void RdmaHw::WakeupOppTokenSender(Ptr<RdmaQueuePair> qp){
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	m_nic[nic_idx].dev->TriggerTransmit();
}

Ptr<OppTokenManager>
RdmaHw::GetOppTokenManagerForInstance(uint32_t probeInstanceId) const
{
	std::map<uint32_t, Ptr<OppTokenManager> >::const_iterator it = m_oppTokenManagers.find(probeInstanceId);
	if (it == m_oppTokenManagers.end())
		return NULL;
	return it->second;
}

Ptr<OppTokenManager>
RdmaHw::GetOppTokenManagerForQp(Ptr<RdmaQueuePair> qp) const
{
	if (qp == NULL)
		return NULL;
	return GetOppTokenManagerForInstance(qp->m_oppProbeInstanceId);
}

void RdmaHw::OnOppTokenReleased(Ptr<RdmaQueuePair> qp){
	if (m_cc_mode != RDMA_CC_MODE_OPP)
		return;
	Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
	if (manager == NULL)
		return;
	Ptr<RdmaQueuePair> liveQp = GetQp(qp->dip.Get(), qp->sport, qp->m_pg);
	if (PeekPointer(liveQp) != PeekPointer(qp))
		return;
	if (qp->snd_nxt >= qp->m_size && !manager->HasToken(qp) && !qp->m_rpingmeshCompleteScheduled){
		qp->m_rpingmeshCompleteScheduled = true;
		QpComplete(qp);
	}
}

void RdmaHw::SendRpingmeshTraceroutePacket(Ptr<RdmaQueuePair> qp, uint32_t seq){
	uint32_t payload_size = GetRpingmeshPayloadSize(CustomHeader::GetStaticWholeHeaderSize());
	...
	ipHeader.SetTtl(m_rpingmeshTracerouteTtl);
	ipHeader.SetTos(RPINGMESH_TRACEROUTE_TOS);
	...
	p->AddHeader(ppp);
	NS_ASSERT_MSG(p->GetSize() == RPINGMESH_PACKET_SIZE, "RPINGMESH traceroute packet size must stay 100 bytes");
...
void RdmaHw::CheckAndSendRpingmeshTraceroute(Ptr<RdmaQueuePair> qp){
	if (m_cc_mode != RDMA_CC_MODE_RPINGMESH ||
			(!m_rpingmeshTracerouteOnMissingAck && !m_rpingmeshTracerouteOnLoopFault &&
			 !m_rpingmeshTracerouteOnCongestionFault))
		return;
	if (qp->m_rpingmeshTracerouteTriggered)
		return;
	Ptr<RdmaQueuePair> liveQp = GetQp(qp->dip.Get(), qp->sport, qp->m_pg);
	if (PeekPointer(liveQp) != PeekPointer(qp))
		return;
	bool shouldSend = false;
	if (m_rpingmeshTracerouteOnMissingAck && qp->m_rpingmeshAckCount < qp->m_size)
		shouldSend = true;
	if (m_rpingmeshTracerouteOnLoopFault && qp->m_rpingmeshLoopFaultSeen)
		shouldSend = true;
	if (m_rpingmeshTracerouteOnCongestionFault && qp->m_rpingmeshCongestionFaultSeen)
		shouldSend = true;
	if (!shouldSend)
		return;
	qp->m_rpingmeshTracerouteTriggered = true;
	qp->m_rpingmeshTracerouteStartNs = Simulator::Now().GetTimeStep();
	for (uint64_t seq = 0; seq < qp->m_size; seq++)
		SendRpingmeshTraceroutePacket(qp, (uint32_t)seq);
}
...
void RdmaHw::AddDefaultRoute(uint32_t intf_idx){
	m_defaultRt.push_back(intf_idx);
}
...
uint32_t RdmaHw::PickRoute(const std::vector<int> &ports, uint32_t hash){
	uint32_t liveCnt = 0;
	for (uint32_t i = 0; i < ports.size(); i++){
		int port = ports[i];
		if (port >= 0 && port < (int)m_nic.size() && m_nic[port].dev != NULL && m_nic[port].dev->IsLinkUp())
			liveCnt++;
	}
	NS_ASSERT_MSG(liveCnt > 0, "We assume at least one NIC is alive");
	uint32_t target = hash % liveCnt;
	for (uint32_t i = 0; i < ports.size(); i++){
		int port = ports[i];
		if (port >= 0 && port < (int)m_nic.size() && m_nic[port].dev != NULL && m_nic[port].dev->IsLinkUp()){
			if (target == 0)
	...
	NS_ASSERT_MSG(false, "Route selection failed");
	return 0;
}
...
bool RdmaHw::CanSendQp(Ptr<RdmaQueuePair> qp){
	if (m_cc_mode != RDMA_CC_MODE_OPP)
		return true;
	Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
	NS_ASSERT_MSG(manager != NULL, "OPP token manager is not configured for this host/instance");
	return manager->TryPrepareToSend(qp);
}

Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	if (qp->m_isRpingmesh){
		bool isOpp = m_cc_mode == RDMA_CC_MODE_OPP;
		uint32_t probeSeq = (uint32_t)qp->snd_nxt;
		uint32_t payload_size = isOpp ? GetOppPayloadSize() : GetRpingmeshPayloadSize(CustomHeader::GetStaticWholeHeaderSize());
		Ptr<Packet> p = Create<Packet> (payload_size);
		if (isOpp){
			Ptr<OppTokenManager> manager = GetOppTokenManagerForQp(qp);
			NS_ASSERT_MSG(manager != NULL, "OPP token manager is not configured for this host/instance");
			OppTokenManager::TokenGrant token = manager->MarkPacketSent(qp, probeSeq);
			NS_ASSERT_MSG(token.valid, "OPP token grant is invalid");
			if (!qp->m_oppProbeFirstSendObserved){
				qp->m_oppProbeFirstSendObserved = true;
				qp->m_oppProbeFirstSendTimeNs = Simulator::Now().GetTimeStep();
			}
			p->AddPacketTag(OppMulticastDecisionTag(token.useUsm));
			OppProbeHeader opph;
			opph.SetTokenIdP(token.tokenId);
			opph.SetTokenSeqNoP(token.tokenSeqNo);
			opph.SetRemainingHopCountP(qp->m_oppRemainingHopCount);
			opph.SetPreviousHopQDelayP(0);
			opph.SetPreviousHopLinkP(0);
			p->AddHeader(opph);
		}
		// add SeqTsHeader
		SeqTsHeader seqTs;
		seqTs.SetSeq (probeSeq);
		seqTs.SetPG (qp->m_pg);
		p->AddHeader (seqTs);
		// add udp header
		UdpHeader udpHeader;
		...
		ppp.SetProtocol (0x0021);
		p->AddHeader (ppp);
		NS_ASSERT_MSG(p->GetSize() == RPINGMESH_PACKET_SIZE, "OPP/RPINGMESH/RDPROBE packet size must stay 100 bytes");

		// update state
		qp->snd_nxt++;
		qp->m_ipid++;
		if (m_cc_mode == RDMA_CC_MODE_RPINGMESH && m_rpingmeshTracerouteOnCongestionFault)
			qp->m_rpingmeshProbeSendTimeNsBySeq[probeSeq] = Simulator::Now().GetTimeStep();
		if (m_cc_mode == RDMA_CC_MODE_RPINGMESH && !m_rpingmeshProbeSentCallback.IsNull())
			m_rpingmeshProbeSentCallback(qp->sip.Get(), qp->dip.Get(), qp->sport, qp->dport,
				qp->m_pg, probeSeq, Simulator::Now().GetTimeStep());
...
void RdmaHw::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> pkt, Time interframeGap){
	...
	if (m_cc_mode == RDMA_CC_MODE_RPINGMESH &&
			(m_rpingmeshTracerouteOnMissingAck || m_rpingmeshTracerouteOnLoopFault ||
			 m_rpingmeshTracerouteOnCongestionFault) &&
			qp->snd_nxt >= qp->m_size && !qp->m_rpingmeshTracerouteScheduled){
		qp->m_rpingmeshTracerouteScheduled = true;
		Simulator::Schedule(MilliSeconds(50), &RdmaHw::CheckAndSendRpingmeshTraceroute, this, qp);
	}
	if (qp->m_isRpingmesh && qp->snd_nxt >= qp->m_size && !qp->m_rpingmeshCompleteScheduled){
		qp->m_rpingmeshCompleteScheduled = true;
		Simulator::Schedule(GetRpingmeshQpCompleteDelay(m_cc_mode), &RdmaHw::QpComplete, this, qp);
	}
}
