...
namespace ns3 {
	...
	Time
		QbbNetDevice::CalculateTxTime(uint32_t bytes) const
	{
		uint64_t bits = static_cast<uint64_t>(bytes) * 8;
		uint64_t bps = m_bps.GetBitRate();
		NS_ASSERT_MSG(bps > 0, "QbbNetDevice data rate must be positive");
		uint64_t txNs = (bits * 1000000000ULL + bps - 1) / bps;
		return NanoSeconds(txNs);
	}
		...
		for (qIndex = 1; qIndex <= fcount; qIndex++){
			...
			if (!paused[qp->m_pg] && qp->GetBytesLeft() > 0 && !qp->IsWinBound()){
				if (m_qpGrp->Get(idx)->m_nextAvail.GetTimeStep() > Simulator::Now().GetTimeStep()) //not available now
					continue;
				if (!m_rdmaCanSendQp.IsNull() && !m_rdmaCanSendQp(qp))
					continue;
				res = idx;
				break;
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddAttribute("SwitchIngressPipelineDelay",
				"Fixed switch ingress pipeline delay before switch-side packet processing.",
				TimeValue(NanoSeconds(400)),
				MakeTimeAccessor(&QbbNetDevice::m_switchIngressPipelineDelay),
				MakeTimeChecker())
			.AddAttribute("SwitchEgressPipelineDelay",
				"Fixed switch egress pipeline delay between dequeue and physical transmit start.",
				TimeValue(NanoSeconds(300)),
				MakeTimeAccessor(&QbbNetDevice::m_switchEgressPipelineDelay),
				MakeTimeChecker())
	...
	QbbNetDevice::QbbNetDevice()
	{
		...
		m_switchEgressTxAvailableTime = Seconds(0);
		m_switchLoopback = false;
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		bool switchNode = m_node->GetNodeType() > 0;
		bool switchEgressPipeline = switchNode && m_switchEgressPipelineDelay.GetTimeStep() > 0;
		if (!switchEgressPipeline && m_txMachineState == BUSY) return;	// Quit if channel busy
		if (switchNode && !m_switchEgressPipelineEvent.IsExpired()) return;
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){
			...
			if (p != 0){
				...
				Time faultDelay = TimeStep(0);
				Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(m_node);
				if (sw != NULL)
					faultDelay = sw->GetFatTreeFaultEgressDelay(m_ifIndex, p);
				if (faultDelay.GetTimeStep() > 0){
					Time txTime = CalculateTxTime(p->GetSize());
					Time txInterval = txTime + m_tInterframeGap;
					Time notifyTime = Simulator::Now() + faultDelay;
					Time txStartTime = notifyTime + m_switchEgressPipelineDelay;
					if (m_switchEgressTxAvailableTime > txStartTime){
						txStartTime = m_switchEgressTxAvailableTime;
						notifyTime = txStartTime - m_switchEgressPipelineDelay;
					}
					m_switchEgressTxAvailableTime = txStartTime + txInterval;
					Simulator::Schedule(notifyTime - Simulator::Now(), &QbbNetDevice::SwitchNotifyDequeueAndTransmitStart, this, p, qIndex);
					m_switchEgressPipelineEvent = Simulator::Schedule(txInterval, &QbbNetDevice::DequeueAndTransmit, this);
				}else{
					FlowIdTag t;
					m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->RemovePacketTag(t);
					m_traceDequeue(p, qIndex);
					if (m_switchEgressPipelineDelay.GetTimeStep() > 0){
						Time txTime = CalculateTxTime(p->GetSize());
						Time txInterval = txTime + m_tInterframeGap;
						Time txStartTime = Simulator::Now() + m_switchEgressPipelineDelay;
						if (m_switchEgressTxAvailableTime > txStartTime)
							txStartTime = m_switchEgressTxAvailableTime;
						m_switchEgressTxAvailableTime = txStartTime + txInterval;
						Simulator::Schedule(txStartTime - Simulator::Now(), &QbbNetDevice::SwitchPipelineTransmitStart, this, p);
						m_switchEgressPipelineEvent = Simulator::Schedule(txInterval, &QbbNetDevice::DequeueAndTransmit, this);
					}else{
						TransmitStart(p);
					}
	...
	void
		QbbNetDevice::SwitchPipelineTransmitStart(Ptr<Packet> p)
	{
		if (!m_linkUp){
			m_traceDrop(p, 0);
			return;
		...
		if (m_txMachineState == BUSY){
			Simulator::Schedule(TimeStep(1), &QbbNetDevice::SwitchPipelineTransmitStart, this, p);
			return;
		}
		TransmitStart(p);
	}
	...
	void
		QbbNetDevice::SwitchNotifyDequeueAndTransmitStart(Ptr<Packet> p, uint32_t qIndex)
	{
		m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
		FlowIdTag t;
		p->RemovePacketTag(t);
		m_traceDequeue(p, qIndex);
		if (m_switchEgressPipelineDelay.GetTimeStep() > 0)
			Simulator::Schedule(m_switchEgressPipelineDelay, &QbbNetDevice::SwitchPipelineTransmitStart, this, p);
		else
			SwitchPipelineTransmitStart(p);
	}
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		m_macRxTrace(packet);
		if (m_node->GetNodeType() > 0 && m_switchIngressPipelineDelay.GetTimeStep() > 0){
			Simulator::Schedule(m_switchIngressPipelineDelay, &QbbNetDevice::ReceiveAfterSwitchIngressPipeline, this, packet);
			return;
		}
		ReceiveAfterSwitchIngressPipeline(packet);
		return;
	}

	void
		QbbNetDevice::ReceiveAfterSwitchIngressPipeline(Ptr<Packet> packet)
	{
		if (!m_linkUp){
			m_traceDrop(packet, 0);
			return;
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		if (m_switchLoopback){
			NS_ASSERT_MSG(m_node->GetNodeType() > 0, "QbbNetDevice switch loopback is only valid on switch nodes");
			Ptr<Packet> looped = p->Copy();
			Simulator::Schedule(txTime, &QbbNetDevice::Receive, this, looped);
			return true;
		}
		NS_ASSERT_MSG(m_channel != 0, "QbbNetDevice without a channel must be a switch loopback device");
   ...
   void QbbNetDevice::TriggerTransmit(void){
	...
	void QbbNetDevice::SetSwitchLoopback(bool loopback){
		m_switchLoopback = loopback;
		if (m_switchLoopback)
			NotifyLinkUp();
	}

	bool QbbNetDevice::IsSwitchLoopback(void) const{
		return m_switchLoopback;
	}

	Time QbbNetDevice::GetSwitchIngressPipelineDelay(void) const{
		return m_switchIngressPipelineDelay;
	}

	Time QbbNetDevice::GetSwitchEgressPipelineDelay(void) const{
		return m_switchEgressPipelineDelay;
	}
