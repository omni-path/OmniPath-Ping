...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport){
	...
	m_var_win = false;
	m_isRpingmesh = false;
	m_rpingmeshCompleteScheduled = false;
	m_rpingmeshTracerouteScheduled = false;
	m_rpingmeshTracerouteTriggered = false;
	m_rpingmeshLoopFaultSeen = false;
	m_rpingmeshCongestionFaultSeen = false;
	m_rpingmeshAckCount = 0;
	m_rpingmeshTracerouteAckCount = 0;
	m_rpingmeshTracerouteStartNs = 0;
	m_rpingmeshMaxProbeAckDelayNs = 0;
	m_oppRemainingHopCount = 0;
	m_oppTokenSourceTorId = 0;
	m_oppTokenDstPodId = 0;
	m_oppProbeInstanceId = 0;
	m_oppProbeFirstSendObserved = false;
	m_oppProbeLastAckObserved = false;
	m_oppProbeFirstSendTimeNs = 0;
	m_oppProbeLastAckTimeNs = 0;
	m_rate = 0;
	m_nextAvail = Time(0);
	mlx.m_alpha = 1;
