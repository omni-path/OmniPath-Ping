...
public:
	...
	bool m_isRpingmesh;
	bool m_rpingmeshCompleteScheduled;
	bool m_rpingmeshTracerouteScheduled;
	bool m_rpingmeshTracerouteTriggered;
	bool m_rpingmeshLoopFaultSeen;
	bool m_rpingmeshCongestionFaultSeen;
	uint32_t m_rpingmeshAckCount;
	uint32_t m_rpingmeshTracerouteAckCount;
	uint64_t m_rpingmeshTracerouteStartNs;
	uint64_t m_rpingmeshMaxProbeAckDelayNs;
	std::map<uint32_t, uint64_t> m_rpingmeshProbeSendTimeNsBySeq;
	std::map<uint32_t, uint32_t> m_rpingmeshTracerouteAckCountByCore;
	std::map<uint32_t, uint64_t> m_rpingmeshTracerouteMaxDelayByCore;
	uint8_t m_oppRemainingHopCount;
	uint32_t m_oppTokenSourceTorId;
	uint32_t m_oppTokenDstPodId;
	uint32_t m_oppProbeInstanceId;
	bool m_oppProbeFirstSendObserved;
	bool m_oppProbeLastAckObserved;
	uint64_t m_oppProbeFirstSendTimeNs;
	uint64_t m_oppProbeLastAckTimeNs;
