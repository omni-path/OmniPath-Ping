...
public:
	...
	typedef Callback<bool, Ptr<RdmaQueuePair> > RdmaCanSendQp;
	RdmaCanSendQp m_rdmaCanSendQp;
...
public:
  ...
  bool Attach (Ptr<QbbChannel> ch);
   ...
   virtual bool IsQbb(void) const;
	   void SetSwitchLoopback(bool loopback);
	   bool IsSwitchLoopback(void) const;
	   Time GetSwitchIngressPipelineDelay(void) const;
	   Time GetSwitchEgressPipelineDelay(void) const;
...
protected:
  ...
  /// Process a received switch packet after the optional ingress pipeline delay.
  virtual void ReceiveAfterSwitchIngressPipeline(Ptr<Packet> p);

  /// Start physical transmission for a packet after it exits the switch pipeline.
  virtual void SwitchPipelineTransmitStart(Ptr<Packet> p);

  /// Notify switch dequeue after a scheduled egress delay, then start transmission.
  virtual void SwitchNotifyDequeueAndTransmitStart(Ptr<Packet> p, uint32_t qIndex);
  ...
  Time m_switchIngressPipelineDelay;
  Time m_switchEgressPipelineDelay;
  EventId m_switchEgressPipelineEvent;
  Time m_switchEgressTxAvailableTime;
  bool m_switchLoopback;
