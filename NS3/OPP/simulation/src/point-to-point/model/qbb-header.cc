...
namespace ns3 {
	...
	void qbbHeader::SetRpingmeshFault(){
		flags |= 1 << FLAG_RPINGMESH_FAULT;
	}
	void qbbHeader::SetRpingmeshTraceroute(){
		flags |= 1 << FLAG_RPINGMESH_TRACEROUTE;
	}
	...
	uint8_t qbbHeader::GetRpingmeshFault() const{
		return (flags >> FLAG_RPINGMESH_FAULT) & 1;
	}
	uint8_t qbbHeader::GetRpingmeshTraceroute() const{
		return (flags >> FLAG_RPINGMESH_TRACEROUTE) & 1;
	}
