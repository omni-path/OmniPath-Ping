...
namespace {
bool g_qbbInstallDebug = false;
QbbHelper::InstallDebugStats g_qbbInstallDebugStats;
...
void
QbbDebugAdd (uint64_t &field, uint64_t start)
{
  if (g_qbbInstallDebug)
    {
      field += QbbDebugNowUs () - start;
    }
}
}

QbbHelper::InstallDebugStats::InstallDebugStats ()
{
  Reset ();
}

void
QbbHelper::InstallDebugStats::Reset ()
{
  links = 0;
  createDeviceUs = 0;
  setAddressUs = 0;
...
void
QbbHelper::SetInstallDebug (bool enabled)
{
  g_qbbInstallDebug = enabled;
}

void
QbbHelper::ResetInstallDebugStats ()
{
  g_qbbInstallDebugStats.Reset ();
}

QbbHelper::InstallDebugStats
QbbHelper::GetAndResetInstallDebugStats ()
{
  InstallDebugStats stats = g_qbbInstallDebugStats;
  g_qbbInstallDebugStats.Reset ();
  return stats;
}
...
QbbHelper::Install (Ptr<Node> a, Ptr<Node> b)
{
  ...
  if (g_qbbInstallDebug)
    {
      g_qbbInstallDebugStats.links++;
    }

  uint64_t t = g_qbbInstallDebug ? QbbDebugNowUs () : 0;
  Ptr<QbbNetDevice> devA = m_deviceFactory.Create<QbbNetDevice> ();
  QbbDebugAdd (g_qbbInstallDebugStats.createDeviceUs, t);

  t = g_qbbInstallDebug ? QbbDebugNowUs () : 0;
  devA->SetAddress (Mac48Address::Allocate ());
  QbbDebugAdd (g_qbbInstallDebugStats.setAddressUs, t);

  t = g_qbbInstallDebug ? QbbDebugNowUs () : 0;
  a->AddDevice (devA);
  QbbDebugAdd (g_qbbInstallDebugStats.addDeviceUs, t);
  ...
  QbbDebugAdd (g_qbbInstallDebugStats.queueUs, t);
  ...
  QbbDebugAdd (g_qbbInstallDebugStats.mpiCheckUs, t);
  ...
  QbbDebugAdd (g_qbbInstallDebugStats.createChannelUs, t);

  t = g_qbbInstallDebug ? QbbDebugNowUs () : 0;
  devA->Attach (channel);
  devB->Attach (channel);
  QbbDebugAdd (g_qbbInstallDebugStats.attachUs, t);

  t = g_qbbInstallDebug ? QbbDebugNowUs () : 0;
  container.Add (devA);
  container.Add (devB);
  QbbDebugAdd (g_qbbInstallDebugStats.containerUs, t);
