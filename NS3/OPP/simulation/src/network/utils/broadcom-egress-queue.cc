...
namespace ns3 {
		...
		BEgressQueue::DoEnqueue(Ptr<Packet> p, uint32_t qIndex)
	{
		...
		NS_ASSERT_MSG(qIndex < fCnt, "BEgressQueue::DoEnqueue: qIndex out of range");
	...
	Ptr<const Packet>
		BEgressQueue::PeekRR(bool paused[], uint32_t &qIndex) const
	{
		NS_LOG_FUNCTION(this);
	...
	{
		NS_ASSERT_MSG(qIndex < fCnt, "BEgressQueue::GetNBytes: qIndex out of range");
