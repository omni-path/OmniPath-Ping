...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
    ...
    .AddAttribute ("BaseRtt",
                   ...
                   MakeUintegerChecker<uint64_t> ())
    .AddAttribute ("OppRemainingHopCount",
                   "Initial OPP remaining hop count.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_oppRemainingHopCount),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("OppTokenSourceTorId",
                   "OPP token source ToR ID field.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_oppTokenSourceTorId),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("OppTokenDstPodId",
                   "OPP token destination PoD ID field.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_oppTokenDstPodId),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("OppProbeInstanceId",
                   "OPP/probing instance ID.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_oppProbeInstanceId),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
void RdmaClient::StartApplication (void)
{
  ...
  rdma->AddQueuePair(m_size, m_pg, m_sip, m_dip, m_sport, m_dport, m_win, m_baseRtt, m_oppRemainingHopCount, m_oppTokenSourceTorId, m_oppTokenDstPodId, m_oppProbeInstanceId, MakeCallback(&RdmaClient::Finish, this));
}
