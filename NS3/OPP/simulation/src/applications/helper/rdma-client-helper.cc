...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint64_t size, uint32_t win, uint64_t baseRtt, uint32_t oppRemainingHopCount, uint32_t oppTokenSourceTorId, uint32_t oppTokenDstPodId, uint32_t oppProbeInstanceId)
{
	...
	SetAttribute ("OppRemainingHopCount", UintegerValue (oppRemainingHopCount));
	SetAttribute ("OppTokenSourceTorId", UintegerValue (oppTokenSourceTorId));
	SetAttribute ("OppTokenDstPodId", UintegerValue (oppTokenDstPodId));
	SetAttribute ("OppProbeInstanceId", UintegerValue (oppProbeInstanceId));
}
